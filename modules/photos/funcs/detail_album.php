...
if( nv_user_in_groups( $global_photo_cat[$category_id]['groups_view'] ) )
{
	...
	$album = $query->fetch();
 	...
 	if( $album['album_id'] > 0 )
	{
		if( defined( 'NV_IS_MODADMIN' ) ) // cap nhat luot xem
		{
	...
	if( $date_added == 0 )
	{
		...
		nv_info_die( $nv_Lang->getModule('error_404_title'), $lang_global['error_404_title'], $lang_global['error_404_content'] . $redirect );
	}
	...
	{
		define( 'NV_COMM_ID', $album_id ); //ID bài viết
...
else
{
	...
	$contents = no_permission( $nv_Lang->getModule('no_permission_album') );
}
