...
$page_title = $nv_Lang->getModule('album');
...
if( ACTION_METHOD == 'deleterows' )
{
	...
	if(  $token == md5( $global_config['sitekey'] . session_id() . $row_id ) )
	{
		...
		if( $data['row_id'] > 0 )
		{
			if( $db->query( 'DELETE FROM ' . TABLE_PHOTO_NAME . '_rows WHERE row_id = ' . $row_id . ' AND album_id=' . $album_id ) )
			{
				...
				$nv_Cache->delMod( $module_name );
				$info['success'] = $nv_Lang->getModule('photo_success_delete');
			}
		}
	}
	...
	{
		$info['error'] = $nv_Lang->getModule('photo_error_delete');
	}
...
if( ACTION_METHOD == 'delete' )
{
	...
	if( ! empty( $del_array ) )
	{
		...
		if( !empty( $a ) )
		{
			$info['success'] = $nv_Lang->getModule('album_success_delete') ;
		}
	}
	else
	{
		$info['error'] = $nv_Lang->getModule('album_error_delete');
	}
	echo json_encode( $info );
	exit();
}
...
if( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit'  )
{
 	...
 	if( $data['album_id'] > 0 )
	{
		...
		$caption = $nv_Lang->getModule('album_edit');
	}
	else
	{
		$caption = $nv_Lang->getModule('album_add');
	}
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		if( empty( $data['name'] ) )
		{
			$error['name'] = $nv_Lang->getModule('album_error_name');	
		}
		if( empty( $data['folder'] ) )
		{
			$error['folder'] = $nv_Lang->getModule('album_error_folder');	
		}
		if( empty( $data['category_id'] ) )
		{
			$error['category'] = $nv_Lang->getModule('album_error_category');	
		}
		
		if( ! empty( $error ) && ! isset( $error['warning'] ) )
		{
			$error['warning'] = $nv_Lang->getModule('album_error_warning');
		}
		...
		if( !empty( $data['folder'] ) && ! is_dir( NV_ROOTDIR . '/' . $currentpath . '/'. $data['folder'] ) )
		{
			...
			if( $mkdir[0] == 0 )
			{
				$error['warning'] = $nv_Lang->getModule('album_error_create_folder'); 
			}else
			{
				...
				if( $data['album_id'] = $db->lastInsertId() )
				{
					try
					{
						...
						foreach( $data['albums'] as $key => $photo )
						{
							...
							{
								$error['error_image'][] = $nv_Lang->getModule('album_error_mime') . ' ' . $photo['basename'];
							...
							if( is_dir( $folder_album ) )
							{
								...
								if( ! $rename )
								{
									$error['warning'] = $nv_Lang->getModule('album_error_copy_photo') . basename( $filePath );
									unset( $data['albums'][$key] );
								}
								else
								{
									...
									if( ! $rename )
									{
										$error .= $nv_Lang->getModule('album_error_copy_photo') . basename( $filePath ) ;
										unset( $data['albums'][$key] );
									}
									else
									{
					...
					{ 
						$error['warning'] = $nv_Lang->getModule('album_error_save');
						//var_dump($e);
					}
